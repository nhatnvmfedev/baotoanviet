	...
	public function generate_children_list( $id, $limit = -1 ) {
		...
		if ( count( $related_posts ) > 0 ) {
			...
			if ( '' != $heading_text ) {
				...
				$heading_text = '<h3 class="sidebar_heading">' . $heading_text . '</h3>' . PHP_EOL;
			}
			...
			foreach ( $related_posts as $rp4wp_post ) {
				...
				if ( 1 == RP4WP::get()->settings->get_option( 'display_image' ) ) {
					if ( has_post_thumbnail( $rp4wp_post->ID ) ) {
						 ...
						 */
						$thumb_size = apply_filters( 'rp4wp_thumbnail_size', 'medium' );
            ...
            $content .= "</a>";
            foreach((get_the_category()) as $category) { $content .='<p class="sidebar_cat u-catSlug-'.$category->slug.'">'. $category->name.'</p>';}
						$content .= "</div>" . PHP_EOL;
					}
				}
        $content .= "<div class='rp4wp-related-post-content'>" . PHP_EOL;

        $content .= "<p class='sidebar_date'>";
        $content .= get_the_date( 'Y.m.d' );
        $content .= "</p>";

				$content .= "<a class='sidebar_title' href='" . get_permalink( $rp4wp_post->ID ) . "'>" . apply_filters( 'rp4wp_post_title', $rp4wp_post->post_title, $rp4wp_post ) . "</a>";
				...
				// 	$content .= "<p>" . apply_filters( 'rp4wp_post_excerpt', $excerpt, $rp4wp_post->ID ) . "</p>";v
        // }


        $content .= "<div class='sidebar_tags'>";
        $posttags = get_the_tags( $rp4wp_post->ID);
        if ($posttags) {
          foreach($posttags as $tag) {
            $content .= '<div class="sidebar_tags_tag u-bg_green"><a href="' . get_tag_link($tag->term_id) . '">'.$tag->name.'</a></div>';
          }
        }
        $content .= "</div>";
